...
extension Date {
    ...
    public static func ks_formatTimeStamp(timeStamp:Int,format:String) -> String {
    ...
    public static func ks_update(format:String) {
        KS_Date_Formatter.dateFormat = format
    }
    
    public static func ks_formatTimeStamp(timeStamp:Int) -> String {
        let confromTimesp = Date(timeIntervalSince1970: TimeInterval(timeStamp))
        return KS_Date_Formatter.string(from: confromTimesp)
    }
    ...
    ///   - format: 格式
    /// - Returns: 时间
    public static func ks_getTimeByStamp(_ timestamp: Int, format: String) -> String {
        var time = ""
        if (timestamp == 0) {
            return ""
        }
    ...
    public static func ks_toTimeStamp(time: String ,format:String) -> Int {
        KS_Date_Formatter.dateFormat = format
